...
CRGB* FX::updateAndFade()
{
  if (mAlpha > 0) { // 0 is invisible
    ...
    if (mAlpha < 255) // 255 no fade
      // fadeToBlackBy(mLeds, NLEDS, 255-mAlpha);
      nscale8_video(mLeds, NLEDS, mAlpha);
    return mLeds;
  }
  return NULL;
}
...
void Fire::update()
{
  ...
  for( byte i = 0; i < NLEDS; i++)
    mHeat[i] = qsub8( mHeat[i],  random8(0, ((mCooling * 10) / NLEDS) + 2));

  //  mHeat from each cell drifts 'up' and diffuses a little
  for( byte k = NLEDS - 1; k >= 2; k--)
...
void Plasma::update()
{
  ...
  int16_t x = -5215;
  int16_t step = 10430 / NLEDS;  // 65536 / (2*pi) => x (-.5 to .5)

  for (byte i=0; i < NLEDS; i++, x += step) {
...
void Cylon::update(){
  ...
  #define FRAC_SHIFT 4
  long pos16 = (ease16InOutQuad(mPos) * (NLEDS - mEyeSize - 1)) >> (16-FRAC_SHIFT);
...
void Cylon::setCmd(const MyCmd &cmd)
{
  switch(cmd.what) {
    ...
    case 'E': mEyeSize = cmd.arg[0]*(NLEDS-1)/255; break;
...
void Cylon::getCmd(const MyCmd &cmd)
{
  switch(cmd.what) {
    ...
    case 'E': answer(cmd, mEyeSize*255/(NLEDS-1)); break;
...
{
  FastLED.addLeds<CHIPSET, LED_PIN, COLOR_ORDER>(mLeds, NLEDS); //.setCorrection(TypicalSMD5050); // = TypicalLEDStrip
  FastLED.setMaxPowerInVoltsAndMilliamps(5, maxmA);
  FastLED.countFPS();
}

bool LedStrip::registerFX(const FX& fx)
{
