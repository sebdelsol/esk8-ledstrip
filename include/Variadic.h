...
// -----------------------------------------------------
// create JoinbySpace(stream, ...) at least 1+2 args

template<class Last> 
  inline Print& JoinbySpace(Print& stream, Last last) 
  { return stream << last; }
...
#define _forEachName(_method) _method##s

#define ForEachMethod(_method)                                      \
    inline void _forEachName(_method)() {};                         \
  template<class First, class... Args>                              \
    inline void _forEachName(_method)(First& first, Args&... args)  \
    {                                                               \
      _method(first);                                               \
      _forEachName(_method)(args...);                               \
    };
